  ...
  function recoverSelectedMusics() {
    if (!musicsSelected || musicsSelected.length === 0) {
      sendMessage({
        severity: 'error',
        summary: 'Error',
        detail: 'It is necessary to select at least one music!',
        sticky: true
      })
      return
    }
    ...
    confirmDialog({
      ...
      accept: () => {
        musicService
          .recoverDeletedMusics(musicsSelected)
          .then(() => {
            sendMessage({
              severity: 'success',
              summary: 'Success',
              detail: 'Musics recovered successfully!',
              sticky: true
            })
          ...
          .catch(() =>
            sendMessage({
              severity: 'error',
              summary: 'Error',
              detail: 'Error when recovered musics!',
              sticky: true
            })
          )
      }
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        dataKey="id"
        selection={musicsSelected}
        onSelectionChange={(e) => setMusicsSelected(e.value)}
        lazy
        first={first}
        onPage={onPage}
        loading={loading}
