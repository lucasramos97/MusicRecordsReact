...
export default function ListMusics() {
  ...
  function checkDeletedMusics() {
    musicService.countDeletedMusics().then(res => {
  ...
  function buttonDeleteMusic(music) {
    return (
      <Button onClick={() => confirmDeleteMusic(music)} icon="pi pi-trash"
  ...
  function confirmDeleteMusic(music) {
    confirmDialog({
      ...
      accept: () => {
        musicService.delete(music.id).then(() => {
          sendMessage({ severity: 'success', summary: 'Success', detail: 'Music deleted successfully!', sticky: true })
          checkDeletedMusics()
          loadMusicList()
        }).catch(() => {
          sendMessage({ severity: 'error', summary: 'Error', detail: 'Error when deleted music!', sticky: true })
        })
      }
    })
  }
    ...
    <div>
      ...
      <div className="table-top-button">
        ...
        {deletedMusicNumbers > 0 && <Button onClick={showDeletedMusic}
          label="Deleted Musics" icon="pi pi-trash" badge={`${deletedMusicNumbers}`}
